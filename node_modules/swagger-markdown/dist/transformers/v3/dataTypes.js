...
const dataTypeResolver = (schema) => {
    ...
    if (schema.getEnum() && schema.getEnum().length > 0) {
        ...
        resolveResults.push(md.string().br(true).concat(md.string('Available values:').bold().concat(` ${enumValues.join(', ')}`)).get());
    }
    if (schema.getDefault()) {
        resolveResults.push(md.string().br(true).concat(md.string('Default:').bold().concat(` ${schema.getDefault()}`)).get());
    }
